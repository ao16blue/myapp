...
export default function HeroImagesUpdateForm(props) {
  const {
    id,
    heroImages,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...heroImagesRecord };
    setImages(cleanValues.images);
    setCatch1(cleanValues.catch);
    setErrors({});
  };
  const [heroImagesRecord, setHeroImagesRecord] = React.useState(heroImages);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id ? await DataStore.query(HeroImages, id) : heroImages;
      setHeroImagesRecord(record);
    };
    queryData();
  }, [id, heroImages]);
  React.useEffect(resetStateValues, [heroImagesRecord]);
  const validations = {
    images: [],
    catch: [],
  };
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            HeroImages.copyOf(heroImagesRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "HeroImagesUpdateForm")}
    >
      <TextField
        label="Images"
        isRequired={false}
